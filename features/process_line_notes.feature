Feature: Process and save text notes from a LINE webhook
  ...
  Scenario: A user sends a note after their session has expired
   ...
   Scenario: A user starts a session by sending a secret code without a space before the note
    Given the secret code "#s1" is configured for the "Group_A_Notes" folder
    When user "U123" sends a text message with "#s1Initial note with no space"
    Then the note "Initial note with no space" should be saved to the "Group_A_Notes" folder